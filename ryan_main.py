...
def main():
    ...
    regression = LogisticRegression(x_train, y_train, num_epochs=5000, beta=0.01, seed=SEED)
    print("Test Accuracy: %f" % regression.test_accuracy(x_test, y_test))

    preds = regression.predict_values(x_test)
    results = []
    for pred in preds:
        if pred[0] > pred[1]:
            results.append(1)
    ...
    for i in xrange(len(results)):
        output.append(x_test[i] + results[i])
    with open("ryan.csv", "w") as outfile:
        writer = csv.writer(outfile)
        for row in output:
            writer.writerow(row)
    print "Written to ryan.csv"
